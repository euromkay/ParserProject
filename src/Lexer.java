...
class Lexer {
			...
			if (switchThisChar) {
				...
				case S_HEX:
					...
					if (Character.isDigit(c) || isHex(c)) {
						...
						ungetChar(c);
						token = new Token(sym.T_INT_LITERAL, baseConversion(buffer, 16));
					}
					break;
				...
				case S_OCTAL:
					...
					if (isOctal(c)) {
						...
						ungetChar(c);
						token = new Token(sym.T_INT_LITERAL, baseConversion(buffer, 8));
					}
					break;
	...
	/* Used to convert HEX or OCTAL number representations to DECIMAL */
	...
	private void loadKeywords() {
		...
		m_htKeywords.put("delete", new Integer(sym.T_DELETE)); /* Phase 0.1: added new, 10/7/2014 J.P. */
		...
		m_htKeywords.put("new", new Integer(sym.T_NEW)); 	/* Phase 0.1: added new, 10/7/2014 J.P. */
